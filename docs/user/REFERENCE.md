...
- **Speed** (*Vitesse* in French, *Velocidad* in Spanish) is the drop-down list (`10`, `200`, `2000` or `20000` instructions per refresh). It sets how many instructions Run executes per *refresh*. A refresh is one redraw of the screen: the emulated screen is repainted about 60 times per second on a typical display, and between two repaints the CPU runs the chosen number of instructions. Hovering the list, or clicking the **?** button next to it (the way to do it on a touch screen), shows this explanation.
