...
| Indexed | `d8(An,Xn)` | `MOVE.L $10(A0,D1.W),D0` | `An` plus an index register, `Xn` (`Dn` or `An`, `.W` sign-extended or `.L`) plus an 8-bit displacement — like Displacement above, both `An` and `Xn` are only read, neither is changed |
