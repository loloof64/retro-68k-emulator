...
| TRAP system calls (print text, draw pixels, clear screen, read gamepad, play tone) | 🚧 6 of 7 planned (`TRAP #0` halt, `TRAP #2` read pixel, `TRAP #3` write pixel, `TRAP #4` clear screen, `TRAP #5` read controller, `TRAP #6` set sound registers) — only `TRAP #1` (print text) is missing, blocked on a font/glyph rendering subsystem this emulator doesn't have yet — see [Reference](./REFERENCE.md) |
