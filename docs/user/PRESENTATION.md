...
| Assembler (turns `.asm` source into runnable bytecode) | 🚧 Library done for nearly every instruction — only `MOVEP`, `MOVE` to CCR/from SR and `ANDI`/`ORI`/`EORI` to CCR are missing (see [Reference](./REFERENCE.md#writing-assembly-source)); connected to the Editor/Debugger |
